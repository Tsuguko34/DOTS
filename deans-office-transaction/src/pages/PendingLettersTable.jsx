...
export default function StickyHeadTable() {
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged((authObj) => {
      ...
      if (authObj) {
        ...
        const q = query(collection(db, "documents"), where("Status", "==", "Pending"), where("forward_To", "==", auth.currentUser.uid))
  ...
  const filterOpen = async () => {
    const fetchData = async () => {
      ...
      const myQuery = query(myCollectionRef, where("Status", "==", "Pending"));
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody sx={{ height: "100%" }}>
            {filteredData
              ...
              .map((row) => (
                <>
                <TableRow hover onClick={() => unread(row.unread, row.id)} role="checkbox" tabIndex={-1} key={row.uID} sx={{cursor: "pointer", userSelect: "none", height: "50px", background: "#F0EFF6",'& :last-child': {borderBottomRightRadius: "10px", borderTopRightRadius: "10px"} ,'& :first-child':  {borderTopLeftRadius: "10px", borderBottomLeftRadius: "10px"} }}>
                  ...
                  <TableCell className={row.unread ? "table-cell unread last" : "table-cell"} align="left">
                    ...
                    <Tooltip title={<Typography sx={{fontSize: "0.8rem"}}>View Document</Typography>} arrow>
                      ...
                      <Tooltip title={<Typography sx={{fontSize: "0.8rem"}}>Approve Document</Typography>} arrow>
                      <CheckIcon
                        style={{
                          fontSize: "30px",
                          color: "#FFF",
                        ...
                        className="cursor-pointer"
                        onClick={() => openApproveReject("accept", row.id)}
                      />
                      </Tooltip>
                      ...
                      <ClearIcon
                        ...
                        onClick={() => openApproveReject("reject", row.id)}
                      />
                      </Tooltip>
                    </Stack>
                  </TableCell>
      ...
      <Dialog open={approveReject} fullWidth maxWidth="sm">
        <DialogTitle className="dialogDisplayTitle">
          <div className="display-title-holder">
            <div className="dialog-title-view">{actionHolder.Action == "accept" ? "Approve" : "Reject"} Document</div>
