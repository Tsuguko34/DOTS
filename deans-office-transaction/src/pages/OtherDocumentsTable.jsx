  ...
  const createIncoming = async (e) => {
    ...
    if(category == "Student Document"){
        documentsToBeAdded =  {
            ...
            urgent: urgent ? 1 : 0,
            unread: 1,
            tracking: newFromDep ? newFromDep : users.find(item => item.uID == newForwardTo)?.full_Name
          };
    }
        ...
        <DialogContent className="form-body">
          <form action="" ref={form} onSubmit={createIncoming} className="form-style">
            ...
            {category === "Student Document" ? (
                ...
                <TextField required className="Text-input" id="fromPer" label="Meeting Details" variant="outlined" onChange={(e) => setNewSched(e.target.value)} inputProps={{ maxLength: 1000 }}/>
                <TextField className="Text-input" id="fromPer" label="Comment/Note" variant="outlined" onChange={(e) => setNewComment(e.target.value)} inputProps={{ maxLength: 1000 }}/>
                <Autocomplete
                className="auto-complete"
                value={newStatus ? newStatus : null}
