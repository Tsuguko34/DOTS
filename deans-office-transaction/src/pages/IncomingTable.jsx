  ...
  const getSignInMethods = async (type, name) => {
    ...
    if (type == "add") {
      log = {
        ...
        log: user.full_Name + ` added an incoming Communication Letter (${name}) ${newFromPer && 'from ' + newFromPer}`,
      }
    } else if (type == "edit") {
      log = {
        date: dayjs().format("MMM D, YYYY h:mm A").toString(),
        log: user.full_Name + ` edited an incoming Communication Letter (${name}) ${editFromPer && 'from ' + editFromPer}`,
      };
...
//FILTEROPEN
  ...
  return (
    ...
    <Paper sx={{ width: "100%", overflow: "hidden", height: "100%", position: "relative" }}>
      ...
      <ComponentToPrint ref={componentRef} dataFromParent="Incoming Communication" filtered={filteredData}/>
      </div>
