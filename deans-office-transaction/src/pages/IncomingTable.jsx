...
export default function StickyHeadTable() {
      ...
      if (signInMethods.includes("google.com")) {
        if (type == "add") {
          await addDoc(logcollectionRef, {
            ...
            log: auth.currentUser.displayName + " added an incoming Communication Letter",
          });
        } else if (type == "edit") {
          await addDoc(logcollectionRef, {
            date: dayjs().format("MMM D, YYYY h:mm A").toString(),
            log: auth.currentUser.displayName + " edited an incoming Communication Letter",
          });
        } else if (type == "delete") {
          await addDoc(logcollectionRef, {
            date: dayjs().format("MMM D, YYYY h:mm A").toString(),
            log: auth.currentUser.displayName + " deleted an incoming Communication Letter",
          });
        } else if (type == "archive") {
          await addDoc(logcollectionRef, {
            date: dayjs().format("MMM D, YYYY h:mm A").toString(),
            log:
              auth.currentUser.displayName + " archived an incoming Communication Letter",
          });
        }
  ...
  const getIncoming = async () => {
    const q = query(
      ...
      where("Remark", "==", "Incoming"), where("document_Type", "==", "Communication"),
