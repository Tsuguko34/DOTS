...
export default function StickyHeadTable() {
  ...
  const getSignInMethods = async (type, name) => {
    ...
    if (type == "add") {
      log = {
        ...
        log: user.full_Name + ` added an outgoing Communication Letter (${name}) ${newFromPer && 'from ' + newFromPer}`,
      }
    } else if (type == "edit") {
      log = {
        date: dayjs().format("MMM D, YYYY h:mm A").toString(),
        log: user.full_Name + ` edited an outgoing Communication Letter (${name}) ${editFromPer && 'from ' + editFromPer}`,
      };
    } else if (type == "archive") {
      log = {
        date: dayjs().format("MMM D, YYYY h:mm A").toString(),
        log: user.full_Name + ` archived an outgoing Communication Letter (${name})`,
      };
    }
    try{
  ...
  const getIncoming = async () => {
    const data = await axios.get(`${port}/documents?remark=Outgoing&type=Communication`)
    if(data.data.success == false){
  ...
  const filterOpen = async () => {
    const fetchData = async () => {
      ...
      const myQuery = query(myCollectionRef, where("Remark", "==", "Outgoing"), where("document_Type", "==", "Communication"));
    ...
    <Paper sx={{ width: "100%", overflow: "hidden", height: "100%", position: "relative" }}>
      ...
      >
        ...
        <ReactToPrint
          ...
          documentTitle="Outgoing Documents"
          pageStyle='
                @page {
                  size: landscape
                  @bottom-left{
          ...
          <TableRow>
              ...
              >
                <Typography sx={{
                  ...
                  alignItems: "center",
                  padding: "0"
                }}> Forwarded By<FilterAltIcon className={filter2|| open3 ? "filter-icon active" : "filter-icon"} aria-label="filter1"  aria-controls={open3 ? 'filter1' : undefined}
                aria-haspopup="true"
      ...
      {emptyResult ? (
        <div className="nothing-holder">
          ...
          <div className="nothing-bottom">
            Add outgoing communication documents.
          </div>
        </div>
      ) : (
        ""
        ...
        <DialogContent className="form-body">
          <form ref={form} action="" onSubmit={createIncoming} className="form-style">
            ...
            <Autocomplete
                ...
                sx={{ width: "100%"}}
                renderInput={(params) => <TextField required value={newReceivedBy} className="auto-complete-text" onChange={(e) => setNewReceivedBy(e.target.value)} {...params} placeholder="Forwarded By" label="Forwarded By" inputProps={{ ...params.inputProps,maxLength: 50 }}/>}/>
            <Autocomplete
              className="auto-complete"
              disablePortal
            ...
            <Autocomplete
              ...
              sx={{ width: "100%"}}
              renderInput={(params) => <TextField required value={newType}  className="auto-complete-text" onChange={(e) => setNewType(e.target.value)} {...params} placeholder="Internal/External" label="Internal/External"/>}/>
        ...
        <DialogContent className="form-body">
          <form action="" onSubmit={updateIncoming} className="form-style">
            ...
            <Autocomplete
                ...
                getOptionLabel={user =>(user.role != undefined && user.full_Name != undefined) ? `(${user.role}) - ${user.full_Name}` : ''}
                sx={{ width: "100%"}}
                renderInput={(params) => <TextField required value={editReceivedBy} className="auto-complete-text" onChange={(e) => setEditReceivedBy(e.target.value)} {...params} placeholder="Forwarded By" label="Forwarded By" inputProps={{ ...params.inputProps,maxLength: 50 }}/>}/>
            <Autocomplete
              className="auto-complete"
              disablePortal
              value={editFromDep}
            ...
            <Autocomplete
              ...
              options={["Internal Communication", "External Communication"]}
              sx={{ width: "100%"}}
              renderInput={(params) => <TextField required value={editType}  className="auto-complete-text" onChange={(e) => setEditType(e.target.value)} {...params} placeholder="Internal/External" label="Internal External"/>}/>
