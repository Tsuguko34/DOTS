...
export default function StickyHeadTable() {
      ...
      if (signInMethods.includes("google.com")) {
        if (type == "add") {
          await addDoc(logcollectionRef, {
            ...
            log: auth.currentUser.displayName + " added an outgoing Memorandum Letter",
          });
        } else if (type == "edit") {
          await addDoc(logcollectionRef, {
            date: dayjs().format("MMM D, YYYY h:mm A").toString(),
            log: auth.currentUser.displayName + " edited an outgoing Memorandum Letter",
          });
        } else if (type == "delete") {
          await addDoc(logcollectionRef, {
            date: dayjs().format("MMM D, YYYY h:mm A").toString(),
            log: auth.currentUser.displayName + " deleted an outgoing Memorandum Letter",
          });
        } else if (type == "archive") {
          await addDoc(logcollectionRef, {
            date: dayjs().format("MMM D, YYYY h:mm A").toString(),
            log:
              auth.currentUser.displayName + " archived an outgoing Memorandum Letter",
          });
        }
  ...
  useEffect(() => {
    const deleteExpiredDocuments = async () => {
        ...
        const snapshot = await getDocs(query(collectionRef, where("document_Type", "in", ["Memorandum", "Memorandum"])));
        ...
        <DialogContent className="dialogDisplay">
          <FormControl variant="outlined" sx={{ marginTop: 1, width: "100%" }}>
            ...
            >
              ...
              <MenuItem value={"IC"}>Internal Memorandum</MenuItem>
              <MenuItem value={"EC"}>External Memorandum</MenuItem>
            </Select>
          </FormControl>
          <FormControl variant="outlined" sx={{ marginTop: 1, width: "100%" }}>
