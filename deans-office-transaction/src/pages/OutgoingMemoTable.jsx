...
export default function StickyHeadTable() {
  ...
  const getSignInMethods = async (type, name) => {
    ...
    if (type == "add") {
      log = {
        ...
        log: user.full_Name + ` added an outgoing Memorandum Letter (${name}) ${newFromPer && 'from ' + newFromPer}`,
      }
    } else if (type == "edit") {
      log = {
        date: dayjs().format("MMM D, YYYY h:mm A").toString(),
        log: user.full_Name + ` edited an outgoing Memorandum Letter (${name}) ${editFromPer && 'from ' + editFromPer}`,
      };
    } else if (type == "archive") {
      log = {
        date: dayjs().format("MMM D, YYYY h:mm A").toString(),
        log: user.full_Name + ` archived an outgoing Memorandum Letter (${name})`,
      };
    }
    try{
  ...
  const filterOpen = async () => {
    const fetchData = async () => {
      ...
      const myQuery = query(myCollectionRef, where("Remark", "==", "Outgoing"), where("document_Type", "==", "Memorandum"));
  ...
  return (
    ...
    <Paper sx={{ width: "100%", overflow: "hidden", height: "100%", position: "relative" }}>
              ...
              >
                <Box sx={{height: "100%", padding: "2vh"}}>
                  <Autocomplete
                    ...
                    sx={{ width: 250, maxHeight: "100px" }}
                    renderInput={(params) => <TextField onChange={(e) => setFilter2(e.target.value == undefined ? "" : e.target.value)} {...params} label="Filter Forwarded By" />}
                  />
                </Box>
              </Menu>
      ...
      {emptyResult ? (
        <div className="nothing-holder">
          ...
          <div className="nothing-bottom">
            Add outgoing memorandum documents.
          </div>
        </div>
      ) : (
        ""
      ...
      <ComponentToPrint ref={componentRef} dataFromParent="Outgoing Memorandum" filtered={filteredData}/>
      </div>
